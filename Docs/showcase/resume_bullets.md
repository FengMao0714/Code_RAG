...
- 引入 Embedding Profile 评测体系，将 `baseline`、`bge-m3`、`e5-base` 的索引 collection、tracker 和 manifest 隔离，避免切换模型时混用旧向量，并输出多模型 Recall@k/MRR/latency 对比报告。
...
- Added an embedding-profile evaluation workflow that isolates ChromaDB collections, trackers, and manifests for `baseline`, `bge-m3`, and `e5-base`, preventing stale-vector contamination during model comparison.
...
4. 为什么 embedding 模型要隔离索引：不同模型的向量空间不兼容，混用旧 collection 会污染评测。
5. 为什么选择 `bge-m3` 做候选：项目问题是中文问题 + 英文/多语言代码标识符的跨语言检索，但最终要由 golden query 指标验证。
6. 为什么 Agent 只读：代码修改需要更强验证链，先做证据定位和风险提示更安全。
7. 为什么强调无副作用命令：调试检索或查看状态不应偷偷 clone/fetch 远程仓库。
